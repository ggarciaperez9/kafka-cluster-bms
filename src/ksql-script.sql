...
CREATE STREAM REJECT_RATE_LIGNE3 WITH (KAFKA_TOPIC='REJECT_RATE_LIGNE3', PARTITIONS=1, REPLICAS=1) AS SELECT COMPONENT_ID, COMPONENTNAME, PRICE, COMPONENTSHAPENAME, LINENAME, MACHINENAME, STATIONTIME, BOARDNAME, BARCODE, REJECT_RATE(COMPONENTABSENCEAFTERPICK_INT, COMPONENTABSENCEBEFOREPLACE_INT, COMPONENTMATERIALDEFECT_INT, TREATMENTERROR_INT, DROPPEDERROR_INT, IDENTERROR_INT, 0, REJECTIDENT_INT, REJECTVACUUM_INT, ACCESSTOTAL_INT) REJECT_RATE, ACCESSTOTAL_INT, PLACEDCOMPONENTS_INT FROM COMPONENT_DETAILS_PRICE_LIGNE3 EMIT CHANGES;
